...
- 开源视频复用性验证（2026-08-05）：4 个 intel-iot-devkit 开源视频（CC-BY 4.0，432p、带音轨、~17.6 MB）零代码改动跑通检测/跟踪/事件/调度/控制面——帧数与视频时长精确对应、JSONL 0 非法、0 ERROR、exit OK；可疑检测（cell phone/boat/tv/chair）经 ORT FP32 逐帧交叉验证全部归因于模型本身（管道零发明）；演示配置 `configs/streams_openvideo.yaml`（文件源，~42s 自然结束），详见 `docs/openvideo_validation.md`
- **Stage 15（Web 可视化仪表盘与一键演示）已完成并验收通过（2026-08-05）**——Control API 加 CORS（`control.cors` 配置可关）+ 零依赖静态仪表盘（实时每路 FPS / 延迟分位数 / 调度状态灯 / 事件流 / 关键帧缩略图 / 安全操作面板）通过 `GET /dashboard` 服务 + 只读端点 `GET /events/recent`、`GET /keyframes`、`GET /keyframes/{name}`（64 KiB 有界尾读、文件名白名单）+ `scripts/demo_run.sh` 一键演示编排（文件源 42s / RTSP 持续两种模式）；单测 294 checks + ctest 9/9；实机全端点验证、浏览器直连演示通过（`configs/demo_dashboard.yaml` 绑定 0.0.0.0 仅限可信网络）；修复 http_server 6 处 send_error 不关闭连接的潜伏缺陷。详见 `docs/stage15_plan.md`
...
- [x] **Stage 15：Web 可视化仪表盘与一键演示**（2026-08-05 完成，`docs/stage15_plan.md`）——CORS + 零依赖仪表盘 + 事件/关键帧只读端点 + demo_run.sh 一键演示；
