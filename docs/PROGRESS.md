...
- **阶段 15（Web 可视化仪表盘与一键演示）：已完成 ✓（2026-08-05）**——CORS（配置可关）+ 零依赖静态仪表盘（每路 FPS/延迟分位数/调度状态灯/事件流/关键帧/安全操作面板，经 `GET /dashboard` 服务）+ 只读端点 `/events/recent`（64 KiB 有界尾读）、`/keyframes`、`/keyframes/{name}`（文件名白名单）+ `scripts/demo_run.sh` 一键演示（文件源 42s / RTSP 持续两种模式）；单测 294 checks（+24）+ ctest 9/9；实机浏览器直连演示通过（`configs/demo_dashboard.yaml`，0.0.0.0 绑定仅限可信网络）；修复 http_server 6 处 send_error 不关闭连接潜伏缺陷。详见 `docs/stage15_plan.md`
...
- **阶段 12（Agent 白名单工具调用、验证、审计与自动回滚）：已完成 ✓（2026-08-04）**——C++ 侧：MetricsRegistry per-frame 推理段延迟环形跟踪（input probe→output probe 按 frame_num 配对、ring 4096/pending 256、watermark 窗口切分、时钟注入）+ `POST /benchmark` 受控测量窗口端点（单飞/审计/stopping_ 优雅退出/global 跨流样本池/complete 标志）+ `/metrics/summary` 延迟分位数字段 + 窗口边界 drop 下溢修复（有符号差值 clamp [0,1]）；Python Agent（独立进程）：7 白名单工具（implementation_plan §64）、DeepSeek 函数调用低频候选计划 + 确定性执行/验证/回滚、LLM 故障降级确定性默认策略、CRITICAL 预检/写前复查/600s 硬 deadline/基线快照回滚+读回比对、双审计链（agent JSONL + 服务端 §16 审计）。验收：ctest 7/7（control_api 255 checks + metrics_registry 29 checks）+ Python 41 用例 0 失败；实机 4 路 RTSP：延迟字段 P95 45-46ms 实测合理、benchmark 正/负路径（400 PARAM_DURATION ×3、404 PARAM_STREAM）、窗口排队语义（9007ms 后正常响应）、场景 A（LLM 在线 before 43.3→42.4→41.7 ms 未达双阈值 ≤36.8/35.3 自动回滚 ×2、LLM 空候选降级照常执行 = 大模型故障不影响闭环证据）、场景 B（--no-llm cam1 FPS 保底 35 实测 29.8→26.1/24.9 触发 batch 填充效应 + P95 双重失败自动回滚）、场景 C（SIGKILL agent 0 ERROR 4 流 RUNNING、未完成变更残留已记录）；服务端审计 benchmark×20/snapshot×7/rollback×12/set_infer_interval×25；RSS 收敛、SIGINT 退出码 0。详见 `docs/stage12_agent.md`
- **阶段 13（INT8 PTQ 与精度回归）：已完成 ✓（2026-08-04）**——自定义校准器（C++17 零 OpenCV 依赖，EntropyCalibrator2 + MinMax 双算法，`apps/calib_generator` + `src/calib/image_reader`）+ 校准数据抽帧（`scripts/extract_calib_frames.py`，667 帧默认 / 991 帧密集实验，4 场景与 nvinfer 同口径预处理）+ 5 个 INT8 engine 变体 Jetson 实机构建（entropy2 / entropy2+FP16 检测头 / minmax ×2，绑定 4x3x384x640→4x84x5040，全部 SHA256 记录）+ 精度回归框架（`scripts/accuracy_math.py` 22 单测 + `scripts/compare_precision.py` 阈值判定 + `scripts/run_ort_reference.py` ORT FP32 2072 帧参考）。实测结论：**entropy2 校准系统性压缩检测置信度（Δconf 均值 0.34、cam1 检测数 -69%、cam2 全灭）证伪**；**检测头 FP16 混合精度（--layerPrecisions=/model.23/*:fp16）受层融合限制无效证伪**；**MinMax 校准有效（Δconf 0.034、cam1 17341≈FP16 17248）但匹配率 0.947/0.705/0.996/0.944 未达保守阈值 0.95**（未匹配 82% 集中在 0.25-0.4 低置信边缘，量化方向安全：无幻觉无类翻转，关键目标留存 bus 0.983/car 0.960/person 0.950）；**按 §16 纪律回退 FP16 交付**；INT8 性能收益实测（干净环境 60s 窗口）：P95 43.44→35.51ms（-18.3%）、P50 -20.2%、P99 -18.4%、drop 0；校准器实机修复 2 个绑定契约缺陷（batch 连续 buffer + cudaMalloc ×batch）；ctest 9/9（新增 image_reader 17 checks + accuracy_math 22 用例）。详见 `docs/stage13_int8.md`
...
- ~~Stage 15：Web 可视化仪表盘与一键演示~~ → **已完成（2026-08-05）**：CORS（配置可关）+ 零依赖仪表盘（流状态/延迟分位数/调度灯/事件流/关键帧/操作面板）+ /events/recent、/keyframes 只读端点 + demo_run.sh 一键演示；单测 294 checks + ctest 9/9 + 实机浏览器直连演示通过；修复 send_error 不关连接 6 处；管道热路径零改动，Docker/Grafana 为 P2 另行请求。见 `docs/stage15_plan.md`
